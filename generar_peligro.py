...
def descargar_xlsx():
    ...
    marca_tiempo = int(datetime.now(timezone.utc).timestamp())

    url = URL_XLSX + "?nocache=" + str(marca_tiempo)
    ...
    if respuesta.status_code != 200:
        raise RuntimeError("No se ha podido descargar el XLSX")
...
def generar_json():
    ...
    print("Municipios generados:", len(municipios))

    for codigo in ("08148", "08270", "08305"):
